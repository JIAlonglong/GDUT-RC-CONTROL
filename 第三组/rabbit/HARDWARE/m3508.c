...
int start=0;
int end=0;
M3508_REAL_INFO M3508_CHASSIS_MOTOR_REAL_INFO[4] = {0};

PID M3508_CHASSIS_MOTOR_PID_RPM[4];	// 4个M3508电机
...
int16_t PICK_TRANSATE_MOTOR_TARGET_RPM;

M3508_CLASS M3508_UP;//抬升电机初始化 CANid为5//是一个类里面有 	M3508_REAL_INFO REAL_INFO; PID MOTOR_PID;	int TARGET_RPM;		int SET_CURRENT;
M3508_CLASS M3508_RAIL;//云台电机初始化 CANid为6 hh
M3508_CLASS M3508_TRANSATE;//传递电机初始化 CANid为7 m2006
M3508_CLASS M3508_PICK;//捡环电机初始化 canid为1 can2
M3508_CLASS M3508_PICK_TRANSATE;//捡环传递电机初始化 canid为8
...
ARM_VELOCITY_PLANNING   YAW_MOVE_1={0,        -2700,          2500,                3500,            0,          0.4,         0.3};
//ARM_VELOCITY_PLANNING   YAW_MOVE_2={0};
ARM_VELOCITY_PLANNING   YAW_DOWN={-2700,           0,         3500,             4500,            0,          0.2,         0.3};	
...
void M3508_Motor_Init(void)
{
	...
	PID_parameter_init(&M3508_RAIL.MOTOR_PID, 10.0, 1.0, 0.0, 16384, 16384,-0.1);
	//需要s型规划的电机
	PID_parameter_init(&M3508_TRANSATE.MOTOR_PID, 10.0, 1.0, 0.0, 16384, 16384,-0.1);
	PID_parameter_init(&M3508_PICK_TRANSATE.MOTOR_PID, 10.0, 1.0, 0.0, 16384, 16384,-0.1);
	PID_parameter_init(&M3508_PICK.MOTOR_PID, 10.0, 1.0, 0.0, 16384, 16384,-0.1);
	//M3508_UP_NORMAL
...
void m3508_update_m3508_info_can1(CanRxMsg *msg)
{
    ...
    case M3508_CHASSIS_MOTOR_ID_1://A轮子
		...
		case M3508_RAIL_ID_6://滑轨机构
		{
			M3508_RAIL.REAL_INFO.ANGLE    =(msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M3508_RAIL.REAL_INFO.RPM      =(msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M3508_RAIL.REAL_INFO.CURRENT  =(msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
			M3508AngleIntegral(&M3508_RAIL.REAL_INFO);
		};break;
		...
		case M3508_PICK_TRANSATE_MOTOR_ID_8://捡环传输机构
		{
			M3508_PICK_TRANSATE.REAL_INFO.ANGLE    =(msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M3508_PICK_TRANSATE.REAL_INFO.RPM      =(msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M3508_PICK_TRANSATE.REAL_INFO.CURRENT  =(msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
			M3508AngleIntegral(&M3508_PICK_TRANSATE.REAL_INFO);
		};break;	
		default: break;
	}
...
void m3508_update_m3508_info_can2(CanRxMsg *msg)
{
	switch(msg -> StdId)  // 检测标准ID
	{
    case M3508_PICK_ID_1://捡环机构
		{ 
			M3508_PICK.REAL_INFO.ANGLE    =(msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M3508_PICK.REAL_INFO.RPM      =(msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M3508_PICK.REAL_INFO.CURRENT  =(msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
			M3508AngleIntegral(&M3508_PICK.REAL_INFO);
		}; break;
		
		case M3508_RAIL_ID_2://先不用
		{ 
			M3508_RAIL.REAL_INFO.ANGLE    =(msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M3508_RAIL.REAL_INFO.RPM      =(msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M3508_RAIL.REAL_INFO.CURRENT  =(msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
...
void chassis_m3508_m2006_send_motor_currents_can1(void)
{
  ...
  tx_message_2.Data[2] = (uint8_t)(M3508_RAIL.REAL_INFO.TARGET_CURRENT >> 8);
  tx_message_2.Data[3] = (uint8_t) M3508_RAIL.REAL_INFO.TARGET_CURRENT;
  ...
  tx_message_2.Data[5] = (uint8_t)(M3508_TRANSATE.REAL_INFO.TARGET_CURRENT);
	tx_message_2.Data[6] = (uint8_t)(M3508_PICK_TRANSATE.REAL_INFO.TARGET_CURRENT >> 8);
  tx_message_2.Data[7] = (uint8_t)(M3508_PICK_TRANSATE.REAL_INFO.TARGET_CURRENT);
...
void chassis_m3508_send_motor_currents_can2(void)
{
	...
	tx_message_1.IDE = CAN_Id_Standard;
	tx_message_1.RTR = CAN_RTR_Data;
	tx_message_1.DLC = 0x04;//0x02代表一个电机
	
	// 配置仲裁段和数据段	
	tx_message_1.StdId = 0x200;  // 用于ID为 1 2 3 4 的电机
  tx_message_1.Data[0] = (uint8_t)(M3508_PICK.REAL_INFO.TARGET_CURRENT >> 8);
	tx_message_1.Data[1] = (uint8_t) M3508_PICK.REAL_INFO.TARGET_CURRENT;
  tx_message_1.Data[2] = (uint8_t)(M3508_RAIL.REAL_INFO.TARGET_CURRENT >> 8);
  tx_message_1.Data[3] = (uint8_t) M3508_RAIL.REAL_INFO.TARGET_CURRENT;
//	tx_message_1.Data[4] = (uint8_t)(M3508_CHASSIS_MOTOR_REAL_INFO[2].TARGET_CURRENT >> 8);
//	tx_message_1.Data[5] = (uint8_t) M3508_CHASSIS_MOTOR_REAL_INFO[2].TARGET_CURRENT;
//	tx_message_1.Data[6] = (uint8_t)(M3508_CHASSIS_MOTOR_REAL_INFO[3].TARGET_CURRENT >> 8);
//	tx_message_1.Data[7] = (uint8_t) M3508_CHASSIS_MOTOR_REAL_INFO[3].TARGET_CURRENT;

	CAN_Transmit(CAN2, &tx_message_1);
}
