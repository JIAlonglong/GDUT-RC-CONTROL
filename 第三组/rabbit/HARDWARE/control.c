...
void World_3wheels(float Vx_RPM, float Vy_RPM, float W_RPM, float theta)
{
  ...
  CHASSIS_MOTOR_TARGET_RPM.MOTOR1_RPM = (kx * Vx_RPM -ky * Vy_RPM - W_RPM * (CHASSIS_X+CHASSIS_Y) ) * M3508_MS_To_RM;
	CHASSIS_MOTOR_TARGET_RPM.MOTOR2_RPM = (kx * Vx_RPM +ky * Vy_RPM - W_RPM * (CHASSIS_X+CHASSIS_Y) ) * M3508_MS_To_RM;
	CHASSIS_MOTOR_TARGET_RPM.MOTOR3_RPM = (kx * Vx_RPM -ky * Vy_RPM + W_RPM * (CHASSIS_X+CHASSIS_Y) ) * M3508_MS_To_RM;
  CHASSIS_MOTOR_TARGET_RPM.MOTOR4_RPM = (kx * Vx_RPM +ky * Vy_RPM + W_RPM * (CHASSIS_X+CHASSIS_Y) ) * M3508_MS_To_RM;
}
...
{
	ad_plan_arm_motor_RPM_UP(*UP_ARM_NOW_MOTION,Plate_angle);
	ad_plan_arm_motor_RPM_YAW(*YAW_ARM_NOW_MOTION,M3508_RAIL.REAL_INFO.REAL_ANGLE);
...
void set_pick_transate_speed(float speed)
{
	TRANSATE_MOTOR_TARGET_RPM = speed;
	PID_incremental_PID_calculation(&M3508_PICK_TRANSATE.MOTOR_PID, M3508_PICK_TRANSATE.REAL_INFO.RPM ,TRANSATE_MOTOR_TARGET_RPM);
	M3508_PICK_TRANSATE.REAL_INFO.TARGET_CURRENT = M3508_PICK_TRANSATE.MOTOR_PID.output;
	chassis_m3508_m2006_send_motor_currents_can1();
}
void set_pick_speed(float speed)
{
	PICK_TRANSATE_MOTOR_TARGET_RPM = speed;
	PID_incremental_PID_calculation(&M3508_PICK.MOTOR_PID, M3508_PICK.REAL_INFO.RPM ,PICK_TRANSATE_MOTOR_TARGET_RPM);
	M3508_PICK.REAL_INFO.TARGET_CURRENT = M3508_PICK.MOTOR_PID.output;
	chassis_m3508_send_motor_currents_can2();
}	