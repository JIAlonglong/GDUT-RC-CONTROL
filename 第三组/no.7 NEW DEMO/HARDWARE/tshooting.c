...
TMOTOR_REAL_INFO MIT_DRIVER_REAL_INFO[2];

void TMOTORSHOOTING_Init(void)
{
	CAN2_Init();
	TMOTOR_control_cmd(TMOTOR_ID1, 0x01);
	TMOTOR_control_cmd(TMOTOR_ID2, 0x01);
//	TMOTOR_control_cmd(TMOTOR_ID3, 0x01);
}
...
速度模式发送函数
*/
void TMOTOR_Speed_Control(u8 ID,float vel)
{
	uint8_t *vbuf;
	vbuf=(uint8_t*)&vel;
...
*/
void TMOTOR_control_cmd(uint8_t ID,uint8_t cmd)
{
	CanTxMsg tx_message;
...
在CAN2中断中不断更新电机的数据
*/
void TMOTOR_update_info(CanRxMsg *msg)//不断更新数据
{
	int p_int;
	int v_int;
	int i_int;
