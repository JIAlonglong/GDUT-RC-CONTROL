...
void MoveInit(void)
{
	...
	PID_parameter_init(&yaw_pid, 0.0 , 2.500 , 0.1 , 180.0, 0, 1);   // 调pid max1.5 2.5 p0.03 0.4  i0.001 0.002 d0.015 0.025   仍然需要调整
...
void LaserLockPoint(int distance_robot , int thetha ,int distance_object,float V_max)//单位：cm
{
	...
	int true_distance=distance_robot1-distance_object1;//注意单位换算 cm->mm
	if(distance_robot==0)//若超出雷达范围
	{ 
		ROBOT_TARGET_VELOCITY_DATA.W_RPM=200;
	}
	else	
	{
